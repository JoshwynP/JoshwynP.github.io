...
// Stable default palette to avoid recreating arrays on each render
const DEFAULT_PALETTE = ['#5227FF', '#FF9FFC', '#B19EEF'];
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const chosenPalette = Array.isArray(colors) && colors.length ? colors : DEFAULT_PALETTE;
    const paletteTex = makePaletteTexture(chosenPalette);
    ...
    class MouseClass {
      ...
      init(container) {
        ...
        // Also listen on window so interactions work even when content overlays the canvas
      ...
      dispose() {
        ...
        window.removeEventListener('mousemove', this._onMouseMove, { passive: true });
...
`;
    ...
    resolution,
    viscous,
    Array.isArray(colors) ? colors.join('|') : 'default',
    autoDemo,
    autoSpeed,
    autoIntensity,
    takeoverDuration,
